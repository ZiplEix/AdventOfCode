...
func getAllStatingPoints(rows []string) []string {
	var result []string
	for i, row := range rows {
		for j, char := range row {
			if char == 'a' {
				result = append(result, toStr(i)+","+toStr(j))
			}
		}
	}
	return result
...
func main() {
	...
	startingPoints := getAllStatingPoints(rows)
	...
	for _, startingPoint := range startingPoints {
		startPoint := strings.Split(startingPoint, ",")
		xS, yS := startPoint[0], startPoint[1]
		start := xS + "," + yS
		end := toStr(xE) + "," + toStr(yE)
		pathWeight, _ := graph.getPath(start, end)
		if (pathWeight < result && pathWeight != 0) || result == 0 {
			result = pathWeight
		}
	}
